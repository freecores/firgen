...
/***************************************************************************
                          structure2vhdl93.cpp  -  description
                             -------------------
...
structure2vhdl93::structure2vhdl93(structureList * sl,implement *im, char * z_SrcFile, char * z_LogFile){
  ...
  if (log)
  {
    fprintf(log,"\n... VHDL93 wizard: version 0.9020312\n");
...
structure2vhdl93::~structure2vhdl93(){

}


int structure2vhdl93::include_entity(){
  int i_success=0;
...
// insert first word of project name
  ...
  strcat(z_entity,"\tflag_clk : integer := 1; \t-- this flag is to enable the 'en' signale for data enable\n");
...
/**  */
int structure2vhdl93::include_arch_head(){
  int i_success=0;
 ...
 //insert coefficient width
  strcat(z_arch_head,"-- library work;\n");
  strcat(z_arch_head,"use work.components_redfir.all; \t-- modify packagename for other technologies\n");
  ...
  strcat(z_arch_head,"\n\n\n -- coefficient structure contains:\n\n");
  for (cnt=0;cnt<(MyStructure->getCoefCnt());cnt++)
  {
      ...
      if   (MyStructure->getCoefSgn(cnt)) sg='-'  ;
      else sg='+';
      sprintf(z,"-- coefficient %d: is float %1.20f    decimal: ",cnt,f);
      strcat(z_arch_head,z);
      sprintf(z," %c%d  signal value id: %d  ;\n",sg,i,n);
      strcat(z_arch_head,z);
...
/**  */
int structure2vhdl93::include_arch_body(){
  ...
  strcat(z_arch_body,"  b_InMapping:block is -- mapping of input signal from std_logic to signed\n");
  strcat(z_arch_body,"  begin\n");
  ...
  strcat(z_arch_body,"  end block b_InMapping; \n\n");


  strcat(z_arch_body,"  b_CoefficientStructure: block is  -- coefficient structure graph\n");
  strcat(z_arch_body,"  begin\n");
  ...
  while (0!=(MyStructure->getModule( fm)))
  {
    ...
    sprintf(z,"   i_%d_node: entity work.%s --line: %d result: %d \n ",cnt,z_module,fm->line,fm->node);strcat(z_arch_body,z);
  ...
  strcat(z_arch_body,"  end block b_CoefficientStructure;\n\n");

//mapping

  strcat(z_arch_body,"  b_MappingUnit: block is  -- mapping coefficents to nedded width\n");
  strcat(z_arch_body,"  begin\n");
  ...
  for (cnt=0;cnt<MyStructure->getCoefCnt();cnt++)
  {
    ...
    if (i!=0){
      ...
      sprintf(z,"    i_%d_map:entity work.REMAP\n",cnt); strcat(z_arch_body,z);
      strcat(z_arch_body,"    generic map (\n");
  ...
  strcat(z_arch_body,"  end block b_MappingUnit;\n\n");


//adderchain
  strcat(z_arch_body,"  b_AdderChain: block is  -- chain to ad taps\n");
  strcat(z_arch_body,"  begin\n\n");
  ...
  for (cnt=0;cnt<MyStructure->getCoefCnt();cnt++)
  {
    ...
    sprintf(z,"    i_%d_chain:entity work.CHAIN\n",cnt); strcat(z_arch_body,z);
    strcat(z_arch_body,"    generic map (\n");
  ...
  strcat(z_arch_body,"  end block b_Adderchain;\n\n");
//outmapping
  strcat(z_arch_body,"  b_Out_Mapping: block is  -- setting out data stream\n");
  strcat(z_arch_body,"  begin\n");
  ...
  strcat(z_arch_body,"  end block b_Out_Mapping; \n\n");

  strcat(z_arch_body,"end architecture ");
